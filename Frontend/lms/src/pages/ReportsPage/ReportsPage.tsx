...
export const ReportsPage: React.FC = () => {
    ...
    // const monthName = new Intl.DateTimeFormat("en-US", { month: "long" }).format(new Date(2000, reportDate.month));

    const handleApplyFilter = (month: number, year: number) => {
        setReportDate({ month, year });
        setIsFilterOpen(false);
        // You would typically trigger a data fetch here based on the new dates
    };
    return (
        <MainLayout>
                    ...
                    <Card className="xl:col-span-5 p-8 border-gray-100 shadow-sm flex flex-col min-h-[500px]">
                        <div className="flex items-center justify-between mb-8">
                            <div className="flex items-center gap-2">
                                ...
                                <h2 className="text-xl font-bold text-gray-900">Popular Genres</h2>
                            </div>
                        </div>

                        <div className="flex-1 flex flex-col justify-between">
                            {/* Top Section: Chart and Labels Side-by-Side */}
                            ...
                            #fb923c 88% 100%
                                    ...
                                    <div className="absolute inset-[22px] bg-white rounded-full flex flex-col items-center justify-center shadow-inner">
                                        <p className="text-3xl font-black text-gray-900">2.8k</p>
                                        <p className="text-[9px] font-bold text-gray-400 uppercase tracking-widest">Total Issues</p>
                                    </div>
                                </div>

                                <div className="flex-1 space-y-3">
                                    <GenreLegendItem label="Academic" percent="42%" color="bg-blue-600" />
                                    <GenreLegendItem label="Fiction" percent="28%" color="bg-purple-500" />
                                    <GenreLegendItem label="Technology" percent="18%" color="bg-green-400" />
                                    <GenreLegendItem label="History" percent="12%" color="bg-orange-400" />
                                </div>
                            </div>

                            {/* Bottom Section: Fills the remaining space with actionable data */}
                            <div className="mt-8 pt-6 border-t border-gray-50">
                                <h3 className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-4">Collection Health</h3>
                                <div className="grid grid-cols-2 gap-4">
                                    <div className="p-4 bg-blue-50/50 rounded-2xl border border-blue-100/50">
                                        <p className="text-[9px] font-black text-blue-600 uppercase tracking-tighter mb-1">Stock Utilization</p>
                                        <div className="flex items-baseline gap-2">
                                            <p className="text-xl font-bold text-gray-900">76%</p>
                                            <p className="text-[10px] font-medium text-blue-500 tracking-tight">Active Loans</p>
                                        </div>
                                    </div>
                                    <div className="p-4 bg-green-50/50 rounded-2xl border border-green-100/50">
                                        <p className="text-[9px] font-black text-green-600 uppercase tracking-tighter mb-1">Return Rate</p>
                                        <div className="flex items-baseline gap-2">
                                            <p className="text-xl font-bold text-gray-900">92%</p>
                                            <p className="text-[10px] font-medium text-green-500 tracking-tight">On-time</p>
                                        </div>
                                    </div>
                                </div>
                    ...
                    <Card className="xl:col-span-7 p-8 border-gray-100 shadow-sm flex flex-col min-h-[500px]">
                        <div className="flex items-center justify-between mb-8">
                            <div className="flex items-center gap-2">
                                ...
                                <h2 className="text-xl font-bold text-gray-900">Fine & Overdue Pulse</h2>
                            </div>
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-x-12 gap-y-10 flex-1">
                            <div className="space-y-8">
                                <div>
                                    <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">Monthly Fine Goal</p>
                                    <div className="flex items-end gap-2">
                                        <p className="text-4xl font-black text-gray-900">Rs.1,240</p>
                                ...
                                <div className="space-y-6">
                                    <ProgressBlock label="Collected Fines" amount="Rs.1,016" percent={82} color="bg-blue-600" />
                                    <ProgressBlock label="Pending (Unpaid)" amount="Rs.224" percent={18} color="bg-orange-500" />
                                </div>
                            </div>

                            <div className="flex flex-col">
                                <h3 className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-6">Overdue Duration</h3>
                                <div className="space-y-5 flex-1">
                                    <OverdueRow label="1-3 Days" count={24} color="bg-yellow-400" total={42} />
                                    <OverdueRow label="4-7 Days" count={12} color="bg-orange-500" total={42} />
                                    <OverdueRow label="1-2 Weeks" count={4} color="bg-red-500" total={42} />
                                    <OverdueRow label="2+ Weeks" count={2} color="bg-gray-900" total={42} />
                                </div>
                            </div>
                            ...
                            <div className="lg:col-span-2 pt-6 border-t border-gray-50">
                                <div className="flex items-center justify-between mb-4">
                                    <div className="flex items-center gap-2">
                                        ...
                                        <h3 className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Priority Follow-ups</h3>
                                    </div>
                                    <button className="flex items-center gap-2 px-4 py-2 bg-[#1a1c24] text-white rounded-lg text-[10px] font-black uppercase tracking-widest hover:bg-black transition-all active:scale-95 shadow-sm">
                                        <Activity size={14} /> Send Reminders
                                    </button>
                                </div>
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                    <PriorityItem name="Ahmed Zayan" book="Deep Work" days={16} fine="Rs.120" />
                                    <PriorityItem name="Fathima Fidha" book="Atomic Habits" days={12} fine="Rs.80" />
                                    <PriorityItem name="Mohamed Yamin" book="Clean Code" days={10} fine="Rs.60" />
                                </div>
                            </div>
                        </div>
                    </Card>
...
const PriorityItem = ({ name, book, days, fine }: any) => (
    <div className="flex items-center justify-between p-3 rounded-xl bg-gray-50 border border-gray-100 hover:border-blue-100 transition-colors group cursor-pointer">
        <div className="space-y-1">
            ...
            <p className="text-[10px] text-gray-500 font-medium truncate w-24">{book}</p>
        </div>
        <div className="text-right">
            <p className="text-xs font-black text-red-600">{days}d</p>
            <p className="text-[9px] font-bold text-gray-400">{fine}</p>
        </div>
        <ChevronRight size={14} className="text-gray-300 group-hover:text-blue-500 transition-colors ml-1" />
    </div>
);
...
const ProgressBlock = ({ label, amount, percent, color }: any) => (
    <div className="space-y-2">
        <div className="flex justify-between items-center">
            ...
            <span className={`text-xs font-black ${percent > 50 ? 'text-blue-600' : 'text-orange-600'}`}>{amount}</span>
        </div>
...
const OverdueRow = ({ label, count, color, total }: any) => (
    <div className="flex items-center gap-4">
        <div className="w-20 text-[10px] font-black text-gray-400 uppercase">{label}</div>
        ...
        <div className="w-6 text-xs font-black text-gray-900 text-right">{count}</div>
    </div>
);