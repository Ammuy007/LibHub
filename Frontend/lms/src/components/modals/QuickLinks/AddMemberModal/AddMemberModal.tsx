...
export const AddMemberModal: React.FC<AddMemberModalProps> = ({
  ...
  React.useEffect(() => {
    if (isOpen) {
      api.getNextMemberId().then(setNextId).catch(console.error);
    }
  }, [isOpen]);
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-gray-900/60 backdrop-blur-sm">
      <div className="bg-white w-full max-w-4xl rounded-[32px] shadow-2xl overflow-hidden flex animate-in zoom-in-95 duration-200">
          ...
          <div className="flex items-center justify-between mb-8">
            <div>
              ...
              <p className="text-sm text-gray-500">
                Register a new reader to the library system.
              </p>
            </div>
          </div>
          ...
          <form className="space-y-6" onSubmit={handleSubmit}>
            ...
            <div className="pt-4 flex gap-3">
              <Button
                ...
                type="button"
                onClick={handleClose}
                className="flex-1 h-14 rounded-xl bg-gray-50/50 hover:bg-gray-100 font-bold text-gray-500"
              >
                Cancel
              </Button>
        ...
        <div className="w-[380px] bg-blue-600 p-10 flex flex-col justify-between relative text-white overflow-hidden group shrink-0">
          ...
          <button
            onClick={handleClose}
            className="absolute top-6 right-6 p-2 hover:bg-white/20 rounded-full text-white/70 transition-colors z-20"
          >
            <X size={20} />
          </button>
          ...
          <div className="relative z-10">
            <div className="flex items-center gap-2 mb-12">
              ...
              <span className="text-[11px] font-black tracking-[0.2em] uppercase opacity-90">
                LibHub
              </span>
            </div>
              ...
              <div className="pt-4 space-y-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs font-bold">{currentMonth}</p>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="relative z-10 mt-auto border-t border-white/10 pt-8">
            <p className="text-[10px] opacity-50 uppercase tracking-widest mb-2">
              {" "}
              Member ID
            </p>
            <div className="bg-black/20 rounded-xl p-4 backdrop-blur-md border border-white/5">
              <p className="text-lg font-mono font-bold tracking-[0.1em] text-blue-100">
                MEM-{new Date().getFullYear()}-{nextId ?? "..."}
              </p>
            </div>
          </div>
        </div>
